...
/****** Object:  View [dbo].[view_SCDM_solicitud_rel_item_budget]    Script Date: 04/06/2025 08:52:15 a. m. ******/
SET ANSI_NULLS ON
GO
...
--select * from SCDM_cat_lovs_valuation_class as v
--join SCDM_cat_tipo_materiales_solicitud as t on t.id = v.id_SCDM_cat_tipo_materiales_solicitud


--SELECT
--    CAST(c.esCM AS INT)                                                AS esCM,
--    m.descripcion                                                      AS Modelo, 
--    MAX(CASE WHEN t.descripcion = 'Rollo'   THEN c.valuation_class END) AS Rollo,
--    MAX(CASE WHEN t.descripcion = 'Cinta'   THEN c.valuation_class END) AS Cinta,
--    MAX(CASE WHEN t.descripcion = 'Platina' THEN c.valuation_class END) AS Platina
--FROM dbo.SCDM_cat_lovs_valuation_class AS c
--INNER JOIN dbo.SCDM_cat_modelo_negocio            AS m  
--    ON c.id_SCDM_cat_modelo_negocio = m.id
--INNER JOIN dbo.SCDM_cat_tipo_materiales_solicitud AS t  
--    ON c.id_SCDM_cat_tipo_materiales_solicitud = t.id
--GROUP BY
--    CAST(c.esCM AS INT),
--    m.descripcion
--ORDER BY
--    CAST(c.esCM AS INT),   -- así primero salen los esCM=0 (TKMM), luego los esCM=1 (CM)
--    m.descripcion;
