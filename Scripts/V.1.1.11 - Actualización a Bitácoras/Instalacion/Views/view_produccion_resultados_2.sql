...
(SELECT 
	...
	,(ISNULL(q.[sum_total_piezas], 0) * ISNULL(q.[Peso Real Pieza Bruto], 0)* ISNULL(q.[Ordenes por pieza], 0)) AS [Peso bruto Total piezas_Kg]
FROM 
(SELECT 
	p.*
	,(p.[Peso Bruto SAP]/NULLIF(CAST(p.[Peso neto SAP] AS float),0))*p.[Peso Real Pieza Neto] AS [Peso Real Pieza Bruto] 
	,p.[sum_total_piezas] * p.[Peso Real Pieza Neto] * p.[Ordenes por pieza] AS [Peso NetoTotal piezas_Kg] 
FROM 
(SELECT  
	t.*
