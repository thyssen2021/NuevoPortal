    ...
    Plugin.prototype = {
        ...
        selectItem: function (that) {
            if (this.options.maxSelect == 0 || this.isBelowMax()) { //allow selection if there's no max or selection is below max
                ...
                this.$element.find('option[value="' + $self.data('value') + '"]').prop('selected', 'selected'); //mark selected on the source, used attr vs prop because clone doesn't carry over selected attr      
                this.updateCount(); //refresh counts
                this.triggerChange(); //trigger change callback    
                console.log($('#subordinados').val());
            }
        },
