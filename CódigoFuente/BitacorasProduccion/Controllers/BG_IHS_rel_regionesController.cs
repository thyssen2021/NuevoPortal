...
namespace Portal_2_0.Models
{
    ...
    public class BG_IHS_rel_regionesController : BaseController
    {
        ...
        public ActionResult Index(string estado, int? version)
        {
            ...
            var bG_IHS_rel_regiones = db.BG_IHS_rel_regiones.Where(x => x.id_ihs_version == version && (String.IsNullOrEmpty(estado)
        ...
        public ActionResult Edit(int? id)
        {
            ...
            ViewBag.id_bg_ihs_plant = AddFirstItem(new SelectList(db.BG_IHS_plantas.Where(x => x.activo == true && x.id_ihs_version == bG_IHS_rel_regiones.id_ihs_version), nameof(BG_IHS_plantas.id), nameof(BG_IHS_plantas.ConcatPlanta), bG_IHS_rel_regiones.id_bg_ihs_plant.ToString()), textoPorDefecto: "-- Seleccionar --");
            ViewBag.id_bg_ihs_region = AddFirstItem(new SelectList(db.BG_IHS_regiones.Where(x => x.activo == true && x.id_ihs_version == bG_IHS_rel_regiones.id_ihs_version), nameof(BG_IHS_regiones.id), nameof(BG_IHS_regiones.descripcion), bG_IHS_rel_regiones.id_bg_ihs_region.ToString()), textoPorDefecto: "-- Seleccionar --");
            return View(bG_IHS_rel_regiones);
        }
        ...
        public ActionResult Edit(BG_IHS_rel_regiones bG_IHS_rel_regiones)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("Index", new { version = bG_IHS_rel_regiones.id_ihs_version });
            }
