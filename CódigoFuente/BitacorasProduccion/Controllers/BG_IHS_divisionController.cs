...
namespace Portal_2_0.Controllers
{
    ...
    public class BG_IHS_divisionController : BaseController
    {
        ...
        // GET: BG_IHS_division     
        public ActionResult Index(int? id_ihs_item, int pagina = 1)
        {
            ...
            var listado = db.BG_IHS_division
                .Where(x=>x.id_ihs_item == id_ihs_item || id_ihs_item ==null)
                .OrderByDescending(x => x.id)
                .Skip((pagina - 1) * cantidadRegistrosPorPagina)
               .Take(cantidadRegistrosPorPagina).ToList();

            var totalDeRegistros = db.BG_IHS_division
                .Where(x => x.id_ihs_item == id_ihs_item || id_ihs_item == null)
                .Count();
            ...
            ViewBag.id_ihs_item = AddFirstItem(new SelectList(db.BG_IHS_item.Where(x=>x.BG_IHS_division.Any()), nameof(BG_IHS_item.id), nameof(BG_IHS_item.ConcatCodigo)), textoPorDefecto: "-- Todos --");
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.id_ihs_item = AddFirstItem(new SelectList(db.BG_IHS_item, nameof(BG_IHS_item.id), nameof(BG_IHS_item.ConcatCodigo), model.id_ihs_item), textoPorDefecto: "-- Seleccionar --");
        ...
        public ActionResult Create(BG_IHS_division bG_IHS_division)
        {
            ...
            {
                if (db.BG_IHS_rel_division.Any(x => x.vehicle == div.vehicle))
                    ModelState.AddModelError("", "Ya existe una división con la misma clave de vehículo: " + div.vehicle);

                if (db.BG_IHS_combinacion.Any(x => x.vehicle == div.vehicle))
            ...
            ViewBag.id_ihs_item = AddFirstItem(new SelectList(db.BG_IHS_item, nameof(BG_IHS_item.id), nameof(BG_IHS_item.ConcatCodigo), bG_IHS_division.id_ihs_item), textoPorDefecto: "-- Seleccionar --");
            return View(bG_IHS_division);
        }
        ...
        public ActionResult Edit(BG_IHS_division bG_IHS_division)
        {
            ...
            foreach (var div in bG_IHS_division.BG_IHS_rel_division)
            {
                if (db.BG_IHS_rel_division.Any(x => x.vehicle == div.vehicle && bG_IHS_division.id != x.id_ihs_division))
                    ModelState.AddModelError("", "Ya existe una división con la misma clave de vehículo: " + div.vehicle);
            ...
            if (ModelState.IsValid)
            {
                ...
                var list = db.BG_IHS_rel_division.Where(x => x.id_ihs_division == bG_IHS_division.id);
                ...
                division.BG_IHS_rel_division = bG_IHS_division.BG_IHS_rel_division;
